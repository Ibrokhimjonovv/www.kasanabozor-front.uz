...
const Notification = ({ number, data, setData }) => {
    ...
    const interval = setInterval(() => {
      setIntervalSeconds((prevInterval) => {
        if (prevInterval >= 15000) {
          setData({ id: data.id, closed: true });
  ...
  return (
    <div className="notification" style={{ "--i": number, "--color": data.type === "error" ? "red" : "#41A58D" }}>
...
const NotificationsDisplay = () => {
  ...
  const handleUpdateData = (data) => {
    if (data.closed) {
      setNotifications((prevNots) => prevNots.filter((value) => value.id !== data.id));
    }
  };
