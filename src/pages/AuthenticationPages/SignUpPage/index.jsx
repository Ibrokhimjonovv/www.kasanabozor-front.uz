...
import RegionSelector from "../../../components/RegionsComponent/regions";
...
function formatDate(dateStr) {
  ...
  if (
    ...
    alert(
      "Tug'ilgan kun sanasini to'g'ri belgilang.\n\nMM-DD-YYYY formati maqul variant"
    );
    return "Invalid date";
  }
...
const SignUpPage = () => {
    ...
    if (
      ...
      try {
        const response = await axios.post(
          `${usersServerUrl}accounts/register/step1/`,
          { phone: formData.phone, password: formData.password1 }
        );
        if (response.data.status === "ok") {
          ...
          setStep(2);
          addNotification(
            "Tasdiqlash ko'di yuborildi!",
            `Siz urinish amalga oshiryotgan telefon raqamiga SMS tasdiqlash xabari yuborildi.`
          );
        } else {
          if (response.data.errors.phone) {
            addNotification(
              "Xatolik yuz berdi!",
              "Siz urinish amalga oshiryotgat telefon raqami allaqachon band etilgan.",
              "error"
            );
          }
        }
        ...
        addNotification(
          "Xatolik yuz berdi!",
          "Servis va Foydalanuvchi o'rtasidagi aloqa uzilib qoldi. Wi-Fi yoki Internet kabelini tekshiring.",
          "error"
        );
      }
    }
  ...
  const handleVerify = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${usersServerUrl}accounts/register/step2/`,
        { code: enteredCode, phone: phone }
      );

      if (response.data.status === "ok") {
        setStep(3);
        addNotification(
          "Tasdiqlash ko'di to'g'ri!",
          "Kiritilgan tasdiqlash ko'di muaffaqiyatli tekshirildi. Endi siz o'z malumotlaringizni kiritishingiz mumkin."
        );
      } else {
        addNotification(
          "Xatolik yuz berdi!",
          "Siz kiritgan tasdiqlash ko'di xato.",
          "error"
        );
      }
    } catch {}
  ...
  const handleFinish = async (e) => {
    ...
    try {
      ...
      if (formData.first_name) {
        formDataC.append("first_name", formData.first_name);
      } else {
        alert("Ism to'ldirilmagan");
      }
      if (formData.last_name) {
        formDataC.append("last_name", formData.last_name);
      } else {
        alert("Familiya to'ldirilmagan");
      }
      if (formData.birthday && !String(formData.birthday).includes("_")) {
        formDataC.append("birthday", formatDate(formData.birthday));
      } else {
        alert("Tug'ilgan kun to'ldirilmagan");
      }
      if (formData.email) {
        formDataC.append("email", formData.email);
      } else {
        alert("Elektron pochta to'ldirilmagan");
      }
      if (formData.activity) {
        formDataC.append("activity", formData.activity);
      }
      if (formData.region) {
        formDataC.append("region", formData.region);
      }
      if (formData.district) {
        formDataC.append("district", formData.district);
      }
      if (formData.gender) {
        formDataC.append("gender", formData.gender);
      }
      if (formData.phone) {
        formDataC.append("phone", formData.phone);
      }
      if (formData.password1) {
        formDataC.append("password", formData.password1);
      }

      const response = await axios.post(
        `${usersServerUrl}accounts/register/step3/`,
        formDataC
      );
      ...
      if (response.data.status === "ok") {
        ...
        addNotification(
          "Muaffaqiyatli yakunlandi!",
          "Siz uchun kiritilgan malumotlar asosida hisob qaydnomasi yaratildi. Loyihani ishlatish bo'yicha Kurslar bo'lima maxsus video darslar mavjud.",
          "success"
        );
        addNotification(
          "Hisobga kirilmagan!",
          "Servisdan foydalanish uchun yangi yaratilgan hisob qaydnomangizga kirib oling.",
          "info"
        );
      } else {
        if (response.data.errors.email) {
          ...
          addNotification(
            "Xatolik yuz berdi!",
            "Siz ishlatmoqchi bo'layotgan elektron pochta allaqchon band qilingan.",
            "error"
          );
        } else {
          addNotification(
            "Kutilmagan xatolik yuz berdi!",
            "Siz amalga oshiryotgan so'rov davrida kutilmagan xatolik yuz berdi. Qayta urinib ko'ring.",
            "error"
          );
        }
      }
    ...
    <div id="signup-cont" className={step >= 3 ? "step-3-cont" : ""}>
      <div className="signup-header">
        ...
        <div className="signup-header-right">
          ...
          <Link to="/auth/sign-in/" id="signup">
            Kirish
          </Link>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {step === 1 && (
            <div className="signup-link">
              Hisobingiz bormi? <Link to="/auth/sign-in/">Kirish</Link>
            </div>
          )}
        </form>
        {step >= 3 && (
