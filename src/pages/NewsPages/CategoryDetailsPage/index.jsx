...
const CategoryDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    if (category && category !== "all") {
      setFc(
        Array.from(newsCategories).find((value) => {
          return parseInt(value.id) === parseInt(category);
        })
      );
      setNews(Array.from(newsList).filter((value) => parseInt(value.category.id) === parseInt(category)));
    } else if (category === "all") {
      setNews(Array.from(newsList));
    }
          ...
          >
            ...
            <ul>
              {newsCategories.map((category, index) => (
                <li key={index}>
                  <Link to={`/news/_/details/${category.id}/`}>{category.title}</Link>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="newsInner">
        <div className="news-documents">
          <div className="news-cards">
            {news.length > 0 ? (news.map((value) => (<Link
                ...
                >
                  <div className="news-card">
                    ...
                    <div className="news-description">{value.description}</div>
