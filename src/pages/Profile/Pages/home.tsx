...
const ProfileDetailPage: React.FC = () => {
  ...
  const display = [
    { name: t("First name"), value: firstName },
    { name: t("Last name"), value: lastName },
    { name: t("Middle name"), value: middleName },
    {
      name: t("Phone number"),
      value: formatPhone({ phone: phone.toString() }),
    },
    { name: t("Email"), value: email || t("Undefined") },
    { name: t("Birthday"), value: birthday || t("Undefined") },
    { name: t("Village, District"), value: [region, district].join(", ") },
    { name: t("Gender"), value: gender === 0 ? t("Male") : t("Female") },
    { name: t("Purposes"), value: purposes },
  ];

  return (
    <>
      <div className="page-title flex w-full items-center justify-between mb-6">
        <h2 className="title text-4xl font-bold">Shaxsiy ma'lumotlarim</h2>

        <Link
          to="/profile/edit/"
          className="ep-button flex items-center justify-center py-1 px-2 border border-brand text-brand rounded-lg transition-all duration-300 hover:text-white hover:bg-brand"
        >
      ...
      <div className="conntent profile-data">
        <div className="image">
        ...
        <div className="text-grid grid grid-cols-2 mt-3 gap-4">
          {display.map((value, index) => (
            <div className="element flex flex-col items-start justify-start" key={index}>
          ...
          <div className="element flex flex-col items-start justify-start col-span-2">
            ...
            <span className="text text-description">{about || t("Undefined")}</span>
          </div>
          <div className="element flex flex-col items-start justify-start col-span-2">
            <span className="title text-lg font-bold">Biografiya</span>
            <span className="text text-description">{biography || t("Undefined")}</span>
          </div>
        </div>
      </div>
    </>
