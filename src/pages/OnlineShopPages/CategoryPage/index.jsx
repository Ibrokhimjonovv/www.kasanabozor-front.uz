...
const CategoryPage = () => {
  ...
  useEffect(() => {
    ...
    if (categories.length) {
      setCategoryData(categories.find((value) => value.meta === category));
      fetchData();
      setLoading(false);
    }
  }, [category, categories]);
    ...
    <div className="products-category">
      <div className="to-back">
        <div className="inner">
          ...
          <span>{loading ? "Loading..." : categoryData.title}</span>
        </div>
      </div>

      <h2 className="title">
        {loading ? "Loading..." : `${categoryData.title} boyicha maxsulotlar`}
      </h2>
      <div className="productsInner">
        {moreProducts.length
          ? moreProducts.map((product) => (
              <Link to={`/online-shop/product/${product.meta}`} key={product.guid}>
          ...
          : !loading && <p>Bu kategoriyada mahsulotlar topilmadi.</p>}
      </div>
    </div>
  );
};
