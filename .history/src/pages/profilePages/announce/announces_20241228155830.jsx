...
const Announces = () => {
  ...
  const nextPage = () => {
    if (currentPage < Math.ceil(announ.length / usersPerPage))
  ...
  const totalPages = Math.ceil(announ.length / usersPerPage);
  const paginate = (pageNumber) => setCurrentPage(pageNumber);
  const startUserIndex = indexOfFirstUser + 1;
  const endUserIndex =
    indexOfLastUser < announ.length ? indexOfLastUser : announ.length;
  useEffect(() => {
    const reveal = () => {
