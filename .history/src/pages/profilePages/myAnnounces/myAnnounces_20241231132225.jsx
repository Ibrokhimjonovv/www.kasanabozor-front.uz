...
const MyAnnounces = () => {
  ...
  const nextPage = () => {
    if (currentPage < Math.ceil(announces.length / usersPerPage))
      setCurrentPage(currentPage + 1);
  };
  const indexOfLastUser = currentPage * usersPerPage;
  const indexOfFirstUser = indexOfLastUser - usersPerPage;
  const currentProducts = announces.slice(indexOfFirstUser, indexOfLastUser);
  const totalPages = Math.ceil(announces.length / usersPerPage);
  const paginate = (pageNumber) => setCurrentPage(pageNumber);
  const startUserIndex = indexOfFirstUser + 1;
  const endUserIndex =
    indexOfLastUser < announces.length ? indexOfLastUser : announces.length;

  // Har bir mahsulotning statusini saqlash uchun state
  const [announceStatuses, setProductStatuses] = useState(
    announces.reduce((acc, product) => {
  ...
  const handleStatusChange = (id) => {
    setAnnounceProductStatuses((prevStatuses) => ({
      ...prevStatuses,
      ...
      <div className="profile-inner">
        ...
        <div className="right">
          ...
          <table className="user-table">
            ...
            <tbody>
              {currentAnnounce.map((product, index) => (
                <tr key={index}>
                  <td>
                    <input type="checkbox" />
                  </td>
                  ...
                  <td>
                    <input
                      ...
                      checked={announceStatuses[product.id]}
                    ...
                    >
                      <span
                        className={announceStatuses[product.id] ? "active" : ""}
                      ></span>
                    </label>
                  </td>
                  <td>
          ...
          {totalPages > 1 && (
            <div className="pagination for-p">
              <div className="soni">
                {announces.length} tadan {startUserIndex} - {endUserIndex} lar
                ko’rsatilmoqda
              </div>
              <div className="users-pages-buttons">
                ...
                <button
                  onClick={nextPage}
                  disabled={
                    currentPage === Math.ceil(announces.length / usersPerPage)
                  }
                >
                  <img src={right} alt="" />
                </button>
