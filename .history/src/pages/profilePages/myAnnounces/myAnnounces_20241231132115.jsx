...
const MyAnnounces = () => {
  ...
  const nextPage = () => {
    if (currentPage < Math.ceil(ann.length / usersPerPage))
      setCurrentPage(currentPage + 1);
  };
  const indexOfLastUser = currentPage * usersPerPage;
  const indexOfFirstUser = indexOfLastUser - usersPerPage;
  const currentProducts = ann.slice(indexOfFirstUser, indexOfLastUser);
  const totalPages = Math.ceil(ann.length / usersPerPage);
  const paginate = (pageNumber) => setCurrentPage(pageNumber);
  const startUserIndex = indexOfFirstUser + 1;
  const endUserIndex =
    indexOfLastUser < ann.length ? indexOfLastUser : ann.length;

  // Har bir mahsulotning statusini saqlash uchun state
  const [productStatuses, setProductStatuses] = useState(
    ann.reduce((acc, product) => {
