...
const EditProfile = () => {
  ...
  const handleRegionChange = (e) => {
    const selectedRegion = regions.find(
      (region) => region.name_uz === e.target.value
    );
    setSelectedRegion(selectedRegion.name_uz);

    // Tumanni va qishloqni tozalash
    setSelectedDistrict("");
    setSelectedVillage("");

    // Filtrlanadigan tumanlarni yangilash
    const filteredDistricts = districts.filter(
      (district) => district.region_id === selectedRegion.name_uz
    );

    setFormData({
      ...formData,
      region: selectedRegion.name_uz,
      district: "",
      village: "",
    });

    // filteredDistrictsni saqlash
    setFilteredDistricts(filteredDistricts);
  };

  const handleDistrictChange = (e) => {
    const selectedDistrict = filteredDistricts.find(
      (district) => district.name_uz === e.target.value
    );
    setSelectedDistrict(selectedDistrict.name_uz);
    ...
    setFormData({
      ...formData,
      district: selectedDistrict.name_uz,
      village: "",
    });

    // Filtrlanadigan qishloqlarni yangilash
    const filteredVillages = villages.filter(
      (village) => village.district === selectedDistrict.name_uz
    );

    // filteredVillagesni saqlash
    setFilteredVillages(filteredVillages);
  };

  const handleVillageChange = (e) => {
    const selectedVillage = filteredVillages.find(
      (village) => village.name_uz === e.target.value
    );
    setSelectedVillage(selectedVillage.name_uz);
    setFormData({ ...formData, village: selectedVillage.name_uz });
  };
